  ...
  pastArray.forEach((char, index) => {
    ...
    if (inputRef) {
      try {
        ...
        console.error(`Error while handling paste: ${error.message}`);
      }
    }
  });
};
...
const onSubmitNewPassword = async (e) => {
 ...
 {!isEmailSent && !isOtpSubmited && (
   ...
   <h1 className="text-white text-2xl font-semibold text-center mb-4">
           ...
           <button className="w-full bg-gradient-to-r from-indigo-500 to-indigo-900 text-white py-2.5 rounded-full mt-3">Submit </button>
   </form>
 )}     
...
{isEmailSent && !isOtpSubmited && (
  ...
  className="bg-slate-900 p-8 rounded-lg shodow-lg w-96 text-sm"
>
  <h1 className="text-white text-2xl font-semibold text-center mb-4">
    Reset Password OTP
  </h1>
